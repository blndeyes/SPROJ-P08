...
// Gets or creates password security record for user
...
// Records failed password attempt and locks account if threshold reached
...
// Checks if new password was recently used (prevents reuse)
...
// Gets current password hash from user record
function get_current_password_hash(user) {
  const current_hash = user.password_hash || user.password || null
  return current_hash
}

// Updates user password and saves previous hash to history
